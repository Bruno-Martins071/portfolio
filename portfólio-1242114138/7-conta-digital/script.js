...
function updateTransactionsList() {
    if (appState.transactions.length === 0) {
        transactionsListEl.innerHTML = '<div class="no-transactions">Não constam transações</div>';
        return;
    }

    transactionsListEl.innerHTML = appState.transactions.map(transaction => `
...
receiveSubmit.addEventListener('click', (e) => {
    ...
    if (!cpfCnpj || !amount || amount <= 0) {
        messageEl.innerHTML = '<div class="error-message">Todos os campos devem ser preenchidos!</div>';
        return;
    }
    ...
    messageEl.innerHTML = '<div class="success-message">Transação realizada com sucesso!</div>';
...
transferSubmit.addEventListener('click', (e) => {
    ...
    if (amount > appState.balance) {
        messageEl.innerHTML = '<div class="error-message">Saldo insuficiente!</div>';
        return;
    }
